...
def get_ws(step: int):
    ...
    return args.ws_schedule[ws_idx], args.ws_schedule[ws_idx] // 2
...
for step in range(train_steps + 1):
    ...
    if last_step or (args.val_loss_every > 0 and step % args.val_loss_every == 0):
        ...
        with torch.no_grad():
            for _ in range(val_steps):
                ...
                val_loss += model(inputs, targets, ws_long, ws_short)
        val_loss /= val_steps
        del val_loader
    ...
    for _ in range(grad_accum_steps):
        inputs, targets = next(train_loader)
        model(inputs, targets, ws_long, ws_short).backward()
