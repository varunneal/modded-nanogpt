...
# -----------------------------------------------------------------------------
# Triton kernel for symmetric matrix multiplication suggested by @byronxu99, optimized by @varunneal

def _get_autotune_configs():
    sizes = [64, 128]
    stage_warp = [(3, 4), (3, 8), (4, 4)]
    cfgs = [
        triton.Config(
            {"BLOCK_SIZE": bs},
            num_stages=stages,
            num_warps=warps,
            # Enables Hopper warp specialization
            num_consumer_groups=2,
            num_buffers_warp_spec=3,
        )
        for bs in sizes
        for warps in stage_warp
    ]
...
def XXT_kernel(
    ...
    off_m = pid_i * BLOCK_SIZE
    off_n = pid_j * BLOCK_SIZE

    if off_n < off_m:
        return

    offs_m = off_m + tl.arange(0, BLOCK_SIZE)
    offs_n = off_n + tl.arange(0, BLOCK_SIZE)
    offs_k = tl.arange(0, BLOCK_SIZE)

    A_batch = A_ptr + pid_b * stride_ab
    C_batch = C_ptr + pid_b * stride_cb

    # Tile pointers into A for rows i and j
    a_ptrs = A_batch + (offs_m[:, None] * stride_a0 + offs_k[None, :] * stride_a1)
    b_ptrs = A_batch + (offs_n[:, None] * stride_a0 + offs_k[None, :] * stride_a1)

    # Alignment/vectorization hints for compiler
    tl.multiple_of(offs_m, BLOCK_SIZE)
    ...
    for _ in tl.range(0, num_k_tiles):
        a = tl.load(a_ptrs)
        b = tl.load(b_ptrs)
        acc += tl.dot(a, tl.trans(b))
        a_ptrs += BLOCK_SIZE * stride_a1
        b_ptrs += BLOCK_SIZE * stride_a1

    c_ptrs = C_batch + (offs_m[:, None] * stride_c0 + offs_n[None, :] * stride_c1)
    tl.store(c_ptrs, acc)

    # Mirror to lower triangle if off-diagonal
    if off_n != off_m:
        c_ptrs_t = C_batch + (offs_n[:, None] * stride_c0 + offs_m[None, :] * stride_c1)
        tl.store(c_ptrs_t, tl.trans(acc))


def XXT(A: torch.Tensor, out: torch.Tensor = None):
    """
    out = A @ A.T for square A: (N,N) or (B,N,N).
    """
    ...
    if out is None:
        out = A.new_empty((N, N) if B == 1 else (B, N, N))

    stride_cb = 0 if (B == 1 and out.ndim == 2) else out.stride(0)

    grid = lambda meta: (B, N // meta["BLOCK_SIZE"], N // meta["block_size"])
    square_symmetric_matmul_kernel[grid](
        A_ptr=A, C_ptr=out,
        B=B, N=N,
        stride_ab=stride_ab, stride_a0=A.stride(-2), stride_a1=A.stride(-1),
        stride_cb=stride_cb, stride_c0=out.stride(-2), stride_c1=out.stride(-1)
    )
...
def bA_plus_cAAT_kernel(
    ...
    for _ in tl.range(0, num_k_tiles):
        ...
        acc += tl.dot(a_blk, tl.trans(b_blk))
        a_ptrs += BLOCK_SIZE * stride_a1
        b_ptrs += BLOCK_SIZE * stride_a1

    acc = acc * c

    a_val_ij_ptrs = A_batch + (offs_m[:, None] * stride_a0 + offs_n[None, :] * stride_a1)
    a_ij = tl.load(a_val_ij_ptrs).to(acc.dtype)
    out_upper = acc + b * a_ij

    c_ptrs = C_batch + (offs_m[:, None] * stride_c0 + offs_n[None, :] * stride_c1)
    tl.store(c_ptrs, out_upper)

    if off_n != off_m:
        a_val_ji_ptrs = A_batch + (offs_n[:, None] * stride_a0 + offs_m[None, :] * stride_a1)
        a_ji = tl.load(a_val_ji_ptrs).to(acc.dtype)
        out_lower = tl.trans(acc) + b * a_ji

        c_ptrs_t = C_batch + (offs_n[:, None] * stride_c0 + offs_m[None, :] * stride_c1)
        tl.store(c_ptrs_t, out_lower)


def bA_plus_cAAT(A: torch.Tensor, b: float, c: float, out: torch.Tensor = None):
    """
    out = b * A + c * (A @ A.T) for square A: (N, N) or (B, N, N)
    """
    assert A.ndim in (2, 3)
    assert A.shape[-1] == A.shape[-2], "A must be square."
    N = A.shape[-1]
    ...
    assert N % 128 == 0, "Requires N % BLOCK_SIZE == 0"
...
optimizer1 = DistAdam(
    scalar_params + head_params + embed_params + smear_gate_params,
    lr=0.008,
    betas=(0.8, 0.95),
    eps=1e-8,
    weight_decay=0.0,
